...
const ROSTERS_PATH = join(ROOT, 'src/data/wc2026/paniniRosters.json')
...
if (teamFilter && teams.length === 0) {
  console.error(`[seed:panini-rosters] Equipo desconocido: ${teamFilter}`)
  process.exit(1)
}
...
async function main(): Promise<void> {
  const label = dryRun ? 'check:panini-rosters' : 'seed:panini-rosters'
  ...
  const result = await syncPaniniRosters(db, file, teams, { dryRun })
  ...
  if (dryRun && result.syncedTeams.length > 0 && teamFilter) {
    console.log(`\nImportar: npm run seed:panini-rosters -- --team=${teamFilter}`)
  }
...
main().catch((e) => {
  ...
  if (msg.includes('default credentials')) {
    console.error(
      '[seed:panini-rosters] Sin credenciales Firebase Admin. Ejecuta: gcloud auth application-default login',
    )
  } else {
    console.error('[seed:panini-rosters] ERROR:', e)
  }
  process.exit(1)
})
