...
function predictionGoalsTeamA(pred: MatchPredictionPayload): number {
  return pred.goalsTeamA ?? pred.goalsHome ?? 0
}

function predictionGoalsTeamB(pred: MatchPredictionPayload): number {
  return pred.goalsTeamB ?? pred.goalsAway ?? 0
}
